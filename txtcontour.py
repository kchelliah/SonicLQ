...
# X = scipy.ndimage.zoom(X, 4)
# Y = scipy.ndimage.zoom(Y, 4)
# Z = scipy.ndimage.zoom(Z, 4)
...
# im = plt.pcolormesh(X, Y, Z, cmap=cmap, norm=norm,antialiased=True)
# plt.axis([X.min(), X.max(), Y.min(), Y.max()])

im = plt.imshow(Z, interpolation='bilinear', cmap=cmap, 
